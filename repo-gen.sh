...
do
	...
	if [ "${arch}" = "any" ]; then
		...
		do
			...
			pushd . &> /dev/null
			cd repo/${archdir}
			repo-add StoneCold-BPi.db.tar.gz "$(basename "${pkg}")"
			repo-add -f StoneCold-BPi.files.tar.gz "$(basename "${pkg}")"
			popd &> /dev/null
		done
	else
		cp $pkg repo/${arch}/
...
if [ -e /media/StoneColdNAS/nas_htdocs ]; then
	rm -rf /media/StoneColdNAS/nas_htdocs/arch/stonecold-bpi
	mkdir -p /media/StoneColdNAS/nas_htdocs/arch
	cp -r repo /media/StoneColdNAS/nas_htdocs/arch/stonecold-bpi
fi
